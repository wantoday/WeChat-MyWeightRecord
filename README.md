...
| 页面 | 作用 |
|---|---|
| 打卡 | 录入体重（默认当天，可改期补登记历史日期；支持斤 / kg 切换），显示与上次的增减、BMI、目标完成度 |
...
⚠️ 注意：手机本地存储会随「清理微信缓存 / 卸载重装」丢失。想跨设备使用就开云同步（见下），需要长期留存也可以定期导出 CSV（见下）。

## 云同步（可选）

默认**不开**，数据只在本机。想让多台设备共享同一份数据：

1. 开发者工具 → 云开发 → 开通（免费基础版），复制环境 ID。
2. 把环境 ID 填进 `miniprogram/config.ts` 的 `CLOUD_ENV_ID`（留空 = 纯本地模式）。
3. 云开发控制台 → 数据库 → 新建集合 `user_data`，权限选「仅管理端可读写」。
4. 右键 `cloudfunctions/weightSync` → **上传并部署：云端安装依赖**。

然后「我的」页会出现「绑定微信号 / 数据同步」。首次点「绑定并同步」即完成绑定 —— 身份取的是微信 openid，同一个微信号在任何设备上都是同一个身份，不需要登录授权。之后换设备用同一微信号点一次同步，就能把数据拉回来。

同一天在两台设备上都改过，以**最后修改的那次**为准；删除也会同步。
